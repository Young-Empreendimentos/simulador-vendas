...
function PessoaCampos({ p, on, tentou }: { p: Pessoa; on: (patch: Partial<Pessoa>) => void; tentou: boolean }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div><label className={label}>CPF</label><input className={mark(req(p.cpf) || (!!p.cpf.trim() && !cpfValido(p.cpf)))} value={p.cpf} onChange={(e) => on({ cpf: e.target.value })} placeholder="000.000.000-00" /></div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
        ...
        <div><label className={label}>Número</label><input className={mark(req(p.docNumero) || (!!p.docNumero.trim() && !!docProblema(p.docTipo, p.docNumero)))} value={p.docNumero} onChange={(e) => on({ docNumero: e.target.value })} placeholder={p.docTipo === 'CNH' ? '11 dígitos' : ''} /></div>
